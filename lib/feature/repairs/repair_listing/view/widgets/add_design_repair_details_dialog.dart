...
import 'package:jewellery_erp_frontend_tab_version/feature/repairs/repair_listing/view_model/repair_design_details_controller.dart';
...
class AddDesignRepairDetailsDialog extends StatelessWidget {
  final String? designId;
  final GetRepairsListingValue repairDetails;
  const AddDesignRepairDetailsDialog({
    super.key,
    this.designId,
  ...
  Widget build(BuildContext context) {
    final AddRepairDesignDetailsViewModel controller =
        Get.put<AddRepairDesignDetailsViewModel>(
          AddRepairDesignDetailsViewModel(),
        );

    if (repairDetails.designImages != null &&
        repairDetails.designImages!.isNotEmpty) {
      controller.populateWithFetchedData(
        repairDetails.designImages!.map((e) => e.toString()).toList(),
      );
    }
  ...
  Widget _buildImageGallery(AddRepairDesignDetailsViewModel controller) {
    return Container(
      height: Get.height * 0.45, // Fixed height for the gallery
      decoration: BoxDecoration(
      ...
      child: Obx(
        () => Column(
          ...
          children: [
            SizedBox(
              height: Get.height * 0.33, // Fixed height for main image area
              child:
            ...
            if (controller.imagePaths.isNotEmpty)
              SizedBox(
                height: Get.height * 0.11, // Fixed height for thumbnail area
  ...
  Widget _buildMainImageDisplay(AddRepairDesignDetailsViewModel controller) {
    return Stack(
      fit: StackFit.expand,
      children: [
        Obx(
  ...
  Widget _buildThumbnail(
    int index,
    AddRepairDesignDetailsViewModel controller,
  ) {
  ...
  Widget _buildThumbnailList(AddRepairDesignDetailsViewModel controller) {
    return Container(
      padding: const EdgeInsets.all(8),
      decoration: BoxDecoration(
  ...
  Widget _buildAddMoreButton(AddRepairDesignDetailsViewModel controller) {
    return InkWell(
  ...
  Widget _buildUploadButton(AddRepairDesignDetailsViewModel controller) {
    return InkWell(
      onTap: () => controller.pickImage(),
      child: Container(
        decoration: BoxDecoration(
  ...
  Widget _buildDesignDetailsForm(AddRepairDesignDetailsViewModel controller) {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DesignOrderDetailsRow(
                          label: "Weight",
                          value: "${repairDetails.netWeight ?? '0'} gm",
                          hasBackground: true,
                        ),
                        DesignOrderDetailsRow(
                          label: "Metal & Purity",
                          value: repairDetails.purity ?? 'N/A',
                        ),
                        DesignOrderDetailsRow(
                          label: "Size",
                          value: repairDetails.size ?? 'N/A',
                          hasBackground: true,
                        ),
                        DesignOrderDetailsRow(
                          label: "Pieces",
                          value: repairDetails.itemDescription ?? 'N/A',
                        ),
                        DesignOrderDetailsRow(
                          label: "Order No",
                          value: repairDetails.repairNumber ?? 'N/A',
                          hasBackground: true,
                        ),
                        SizedBox(height: Get.height * 0.01),
                        const CustomText(
  ...
  Widget _buildFooter(AddRepairDesignDetailsViewModel controller) {
