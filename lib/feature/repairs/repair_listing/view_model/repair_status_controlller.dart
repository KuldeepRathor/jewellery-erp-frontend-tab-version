...
import 'package:jewellery_erp_frontend_tab_version/feature/repairs/repair_listing/view/widgets/check_sales_details_repair_dialog.dart';
import 'package:jewellery_erp_frontend_tab_version/feature/repairs/repair_listing/view/widgets/check_tagging_repair_item_dialog.dart';
...
class RepairStatusController extends GetxController {
  ...
  Future<void> handleStatusUpdate() async {
    ...
    log('Handling status update to: $apiStatus for repair: $repairId');
    ...
    if (apiStatus.toLowerCase() == 'ready') {
      ...
      // Show tagging dialog
      await Get.dialog(
        CheckTaggingRepairItemDialog(repairId: repairId, status: apiStatus),
      ...
      await Get.dialog(
        CheckSalesDetailsRepairDialog(repairId: repairId, status: apiStatus),
        barrierDismissible: false,
      );
    } else {
      await updateStatus();
  ...
  // Add new method to fetch tagging details
  ...
  Future<void> updateStatusWithSalesId() async {
    try {
      ...
      await _repository.assignRepairStatus(
        id: repairId,
        status: apiStatus,
  ...
  Future<void> updateStatus() async {
    try {
      ...
      await _repository.assignRepairStatus(id: repairId, status: apiStatus);
