...
import 'package:jewellery_erp_frontend_tab_version/feature/material_in_out/material_in_view/view/material_in_view.dart';
...
class MaterialInListingViewModel extends GetxController {
  ...
  final customerListingResponse = Rx<ApiResponse<GetMaterialInListingResponse>>(
  ...
  void editCustomer(String customerId) {
    ...
    sidebarController.navigateToWidget(
      newChild: ViewMaterialInPage(id: customerId),
    );
  }
              ...
              onSelected: (String value) {
                switch (value) {
                  case 'View':
                    ...
                    break;
                  case 'Cancel':
                    PermissionGuardUtil.withActionPermission(7054, () {
                      Get.dialog(
                        CancelPaymentDialog(
                          subtitle:
                              'Are you sure you want to cancel this material in record?',
                          onYesPressed: () async {
                            await cancelMaterialInRecord(customerDetail!.id!);
                          },
                        ),
                      );
                    });
  ...
  Future<void> cancelMaterialInRecord(String recordId) async {
    try {
      await _inventoryRepository.cancelMaterialInRecord(recordId);
      // Refresh the list after successful cancellation
      await getCustomerListingDetails(resetList: true);
    } catch (e) {
      log("Error cancelling material in record: $e");
      showErrorToast(
        message: 'Failed to cancel material in record: ${e.toString()}',
      );
    }
  }
  ...
  Future<void> getCustomerListingDetails({
    ...
    try {
      final response = await _inventoryRepository.getMaterialInListing(
      ...
      if (resetList) {
        ...
        List<GetMaterialInListingValue> newData = [
          ...currentData,
          ...response.values ?? [],
        ];
