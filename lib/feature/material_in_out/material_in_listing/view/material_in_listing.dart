...
import 'package:jewellery_erp_frontend_tab_version/feature/material_in_out/material_in_listing/view_model/material_in_listing_view_model.dart';
...
class MaterialInListing extends StatefulWidget {
  final bool? wantBackButton;
  const MaterialInListing({super.key, this.wantBackButton = false});

  @override
  State<MaterialInListing> createState() => _MaterialInListingState();
}

class _MaterialInListingState extends State<MaterialInListing> {
  // final controller =
  //     Get.put<CustomerListingViewmodel>(CustomerListingViewmodel());
  final MaterialInListingViewModel controller = Get.put(
    MaterialInListingViewModel(),
  );
  ...
  void onNewMaterialInTap() {
    PermissionGuardUtil.withActionPermission(7053, () {
      ...
      sidebarController.navigateToWidget(newChild: const MaterialInPage());
    });
  }
  ...
  Widget _buildCustomerTable(MaterialInListingViewModel controller) {
    return Expanded(
      child: Container(
