...
import 'package:jewellery_erp_frontend_tab_version/feature/material_in_out/material_out_view/view_model/view_material_out_view_model.dart';
...
class ViewMaterialOutPage extends StatefulWidget {
  final String id;

  const ViewMaterialOutPage({super.key, required this.id});

  @override
  State<ViewMaterialOutPage> createState() => _ViewMaterialOutPageState();
}

class _ViewMaterialOutPageState extends State<ViewMaterialOutPage> {
  final ViewMaterialOutController controller = Get.put(
    ViewMaterialOutController(),
  );

  @override
  void initState() {
    super.initState();
    controller.getMaterialOutById(id: widget.id);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Obx(() {
        switch (controller.getMaterialOutByIdResponse.value.status) {
          case Status.LOADING:
            return const Center(child: CircularProgressIndicator());
          case Status.COMPLETED:
            final data = controller.getMaterialOutByIdResponse.value.data!;
            return SingleChildScrollView(
              child: Column(
                children: [
                  HeaderWidget(
                    header: "View Material Out",
                    wantBackButton: true,
                    onBackButtonTap: () {
          ...
          case Status.ERROR:
            return Center(
              child: Text(
                controller.getMaterialOutByIdResponse.value.message ?? '',
              ),
            );
          default:
...
class MaterialOutItemDetailsWidget extends StatelessWidget {
  final List<LineItem> lineItems;

  const MaterialOutItemDetailsWidget({super.key, required this.lineItems});
        ...
        child: Column(
          ...
          children: [
            ...
            InkWell(
              onTap: () {
                SidebarController sidebarController = Get.find();
                // sidebarController.selectSubItem(1, 0);
                sidebarController.selectSubMenuItem('sales', 'sales_invoice');
              },
              child: const Padding(
