...
// Make sure this class name exactly matches what's used in the rest of the app
class MaterialOutVendorBillDetailsController extends GetxController {
  ...
  final RxString partyType = ''.obs; // To track whether it's vendor or customer
  ...
  Future<void> loadSequencesDropdown({
    ...
    try {
      ...
      final response = await _inventoryRepository.getSequencesDropdown(
  ...
  // This method should be called when the party type changes
  void setPartyType(String type) {
    partyType.value = type;
    // Fetch the appropriate invoice number based on party type
    fetchNextInvoiceNumber();
  }

  // Automatically determine the invoice type based on partyType
  String getInvoiceType() {
    if (partyType.value == 'vendor') {
      return "material_out_number_vendor";
    } else if (partyType.value == 'customer') {
      return "material_out_number_customer";
    } else {
      // Default to vendor if not set
      log('Warning: Party type not set, defaulting to vendor');
      return "material_out_number_vendor";
    }
  }

  Future<void> fetchNextInvoiceNumber({String? invoiceType}) async {
    try {
      final type = invoiceType ?? getInvoiceType();
      log('Fetching next invoice number for type: $type');

      final nextInvoiceNumber = await _inventoryRepository.nextSequence(
        invoiceType: type,
      );

      invoiceNumber.value = nextInvoiceNumber;
      log('Next invoice number fetched: $nextInvoiceNumber');
    } catch (e) {
  ...
  void clearControllers() {
    ...
    setDefaultDates();
    // Don't clear the invoice number as it might be needed
  }
}
