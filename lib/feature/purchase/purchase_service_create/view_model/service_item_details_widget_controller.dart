...
class ServiceItemDetailsTableData {
  String sn;
  ...
  List<FocusNode> tableFocusNodes = [FocusNode(), FocusNode()];

  ServiceItemDetailsTableData({
    required this.sn,
    required this.code,
...
class ServiceItemDetailsWidgetController extends GetxController {
  final headers = [
    'Sn',
    'Ornament',
    // 'Ornament Type',
  ...
  final totalColumnWidths = [2.370, 1.135, 0.1];
  ...
  final RxList<ServiceItemDetailsTableData> controllers =
      <ServiceItemDetailsTableData>[].obs;
  ...
  Future<void> getCodeList() async {
    await _inventoryViewmodel.getAllOrnaments(isService: true);
  ...
  void updateTotals() {
    ...
    totalHeadersValue.value = [
      ...
      // totalMC.toStringAsFixed(3),
      // totalStone.toStringAsFixed(3),
      // totalRate.toStringAsFixed(3),
  ...
  Future<bool> validateRow() async {
    try {
      controllers.removeWhere(
        (element) => element.code.text.isEmpty || (element.amount.text.isEmpty),
      );
