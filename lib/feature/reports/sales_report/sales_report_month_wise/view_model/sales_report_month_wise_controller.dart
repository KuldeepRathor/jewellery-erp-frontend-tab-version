...
class SalesReportMonthWiseController extends GetxController {
  ...
  final getSalesReportMonthWiseResponse =
      Rx<ApiResponse<GetSalesReportMonthWiseReponse>>(
  ...
  List<TableRow> buildRows(BuildContext context) {
    return List.generate(
      getSalesReportMonthWiseResponse.value.data?.values?.length ?? 0,
  ...
  TableRow buildTableRow(int index) {
    List<Widget> cells = [];
    final salesReport = getSalesReportMonthWiseResponse.value.data?.values
        ?.elementAt(index);
    ...
    for (int i = 0; i < headers.length; i++) {
      ...
      switch (i) {
        case 0:
          cellContent = salesReport?.monthName.toString() ?? "-";
  ...
  Future<void> getSalesReportMonthWiseListingDetails({
    ...
    try {
      ...
      final response = await estimationRepository
          .getSalesReportMonthWiseListing(
      ...
      if (resetList) {
        ...
        List<GetSalesReportMonthWiseValue> newData = [
          ...currentData,
          ...response.values ?? [],
        ];
      ...
      // Since GetSalesReportMonthWiseReponse doesn't have pagination property,
  ...
  void setSearchQuery(String query) {
    ...
    _debouncer.run(() {
      getSalesReportMonthWiseListingDetails(resetList: true, isSearch: true);
    });
  }

  Future<void> loadMoreItems() async {
    if (!isLoadingMore.value && hasMorePages.value) {
      await getSalesReportMonthWiseListingDetails();
    }
  }
  ...
  void getApprovalListingDetails({bool resetList = false}) {
    getSalesReportMonthWiseListingDetails(resetList: resetList);
  }
}
