...
class PurchaseReturnInvoiceItemDetailsWidget extends StatelessWidget {
  final List<PurchaseReturnResponseLineItem> lineItems;

  const PurchaseReturnInvoiceItemDetailsWidget({
    super.key,
                  ...
                  if (headerEntry.value == "Stone (₹)") {
                    List<StoneDetailsTableData> details =
                        lineItems[index].lineStones
                            ...
                            .toList() ??
                        [];
                    log("The stone name is ${details.firstOrNull?.name.text}");
                    Get.dialog(
                      StoneDetailsDialog(
  ...
  String getWastageValues(PurchaseReturnResponseLineItem lineItem) {
  ...
  String _getCellValue(
    int headerIndex,
    PurchaseReturnResponseLineItem item,
    int rowIndex,
  ) {
    switch (headerIndex) {
      case 0:
  ...
  double getVADoubleValue(PurchaseReturnResponseLineItem lineItem) {
    if (lineItem.va != null) {
