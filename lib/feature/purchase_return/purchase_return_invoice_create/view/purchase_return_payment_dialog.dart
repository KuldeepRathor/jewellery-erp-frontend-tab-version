...
import 'package:jewellery_erp_frontend_tab_version/feature/purchase_return/purchase_return_invoice_create/view_model/purchase_return_payments_detail_controller.dart';
...
class PurchaseReturnPaymentDetailsTableData {
  ...
  PurchaseReturnPaymentDetailsTableData({
...
class PurchaseReturnPaymentDetailsDialog extends StatefulWidget {
  const PurchaseReturnPaymentDetailsDialog({super.key});

  @override
  State<PurchaseReturnPaymentDetailsDialog> createState() =>
      _PurchaseReturnPaymentDetailsDialogState();
}

class _PurchaseReturnPaymentDetailsDialogState
    extends State<PurchaseReturnPaymentDetailsDialog> {
  final PurchaseReturnPaymentDetailsController controller = Get.put(
    PurchaseReturnPaymentDetailsController(),
  );

  final PurchaseReturnItemDetailsController itemDetailsController =
      Get.find<PurchaseReturnItemDetailsController>();
  final PurchaseReturnInvoiceViewmodel purchaseInvoiceViewmodel =
      Get.find<PurchaseReturnInvoiceViewmodel>();
        ...
        actions: <Type, Action<Intent>>{
          ...
          SaveStoneDetailsIntent: CallbackAction<SaveStoneDetailsIntent>(
            onInvoke: (intent) async {
              ...
              await controller.validateAndPostPurchaseReturnInvoice();
  ...
  Widget _buildHeader() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          const CustomText(
            text: 'Purchase Return Payment Details',
            color: primaryColor,
            fontSize: 16,
          ...
          onChanged: (newValue) {
            if (newValue.isNotEmpty) {
              ...
              if (double.tryParse(newValue) != null) {
                controller.getSubTotalWithHallmarkCharges();
                controller.calculateGstvalues();
                controller.getRoundOffTotal();
              }
            } else {
              // If input is empty, treat as zero
              controller.hallMarkChargesController.text = "0";
              controller.getSubTotalWithHallmarkCharges();
              controller.calculateGstvalues();
              controller.getRoundOffTotal();
            }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: TextFormField(
              ...
              readOnly: readOnly,
              inputFormatters: [
                AmountInputFormatter(), // Restrict to 2 decimal places
              ],
      ...
      child: Row(
        ...
        children: [
          Material(
            ...
            child: InkWell(
              onTap: () => controller.validateAndPostPurchaseReturnInvoice(),
