...
class PurchaseReturnPartyDetailsWidget extends StatelessWidget {
  PurchaseReturnPartyDetailsWidget({
    super.key,
  ...
  final PurchaseReturnPartyDetailsController controller =
  ...
  // // Handle keyboard events for the invoice dropdown
  // KeyEventResult handleInvoiceKeyEvent(FocusNode node, KeyEvent event) {
  //   if (event is KeyDownEvent) {
  //     if (event.logicalKey == LogicalKeyboardKey.enter) {
  //       // If Enter is pressed without Shift
  //       if (!HardwareKeyboard.instance.isShiftPressed) {
  //         // If we have options, select the first one
  //         if (controller.invoiceOptions.isNotEmpty) {
  //           controller.setSelectedInvoice(controller.invoiceOptions.first);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SearchDropdownMenu(
                          ...
                          focusNode: focusNode,
                          nextFocusNode: controller.invoiceFocusNode,
                        ),
                      ],
                    ),
                  ),
                      ...
                      children: [
                        ...
                        Obx(() {
                          return SizedBox(
                            ...
                            >(
                              controller:
                                  controller.invoiceSearchController.value,
                              focusNode: controller.invoiceFocusNode,
                              items: controller.invoiceOptions,
                              getDisplayValue:
                                  (invoice) => invoice.invoiceNumber ?? 'N/A',
                              onSelected: (value) async {
                                controller.setSelectedInvoice(value);
                                if (nextFocusNode != null) {
                                  nextFocusNode?.requestFocus();
                                }
                              },
                              customOptionsBuilder: (textEditingValue) async {
                                await controller.searchInvoices(
                                  textEditingValue.text,
                                );
                                return controller.invoiceOptions.toList();
                              },
...
class SearchDropdownMenu extends StatelessWidget {
  ...
  Future<void> _onItemSelected(dynamic item, bool isPurchase) async {
    if (item is CustomerSearchValue || item is VendorSearchValue) {
      ...
      controller.setSelectedParty(item);
      ...
      // Focus on the invoice dropdown after party selection
      if (nextFocusNode != null) {
        nextFocusNode?.requestFocus();
      }
