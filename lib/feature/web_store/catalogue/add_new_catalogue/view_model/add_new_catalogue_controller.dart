...
class AddNewCatalogueController extends GetxController {
  ...
  Future<void> saveCatalogue({required bool isEdit, String? catId}) async {
    try {
      ...
      if (rateController.text.isEmpty) {
        showErrorToast(message: "Please enter rate");
        return;
      }
      ...
      try {
        ...
        if (!isEdit && catId == null) {
          response = await _inventoryRepository.createCatalogue(
            createCatalogueRequest,
          );
        } else if (isEdit && catId != null) {
          response = await _inventoryRepository.editCatalogue(
            createCatalogueRequest,
            catId,
          );
        }
