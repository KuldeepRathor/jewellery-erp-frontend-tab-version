...
class BannerFourXFourUploadController extends GetxController {
  ...
  final bannersResponse = Rx<ApiResponse<GetAllBannersFourXFourResponse>>(
  ...
  final List<Map<String, String>> bannerPositions = [
    {
      ...
      'position': 'Home Page',
      'subtext': 'Banner 1',
      'fullPosition': 'Home Page Banner 1',
      'size': '590 X 223',
      'aspectRatio': '2.64',
    },
    {
      ...
      'position': 'Home Page',
      'subtext': 'Banner 2',
      'fullPosition': 'Home Page Banner 2',
      'size': '590 X 223',
      'aspectRatio': '2.64',
    },
    {
      ...
      'position': 'Home Page',
      'subtext': 'Banner 3',
      'fullPosition': 'Home Page Banner 3',
      'size': '590 X 223',
      'aspectRatio': '2.64',
    },
    {
      ...
      'position': 'Home Page',
      'subtext': 'Banner 4',
      'fullPosition': 'Home Page Banner 4',
      'size': '590 X 223',
      'aspectRatio': '2.64',
    },
  ];
  ...
  final RxMap<String, GetAllBannersFourXFourValue> originalBannerData =
      <String, GetAllBannersFourXFourValue>{}.obs;
  ...
  Future<void> fetchBanners() async {
    try {
      ...
      final response = await _webstoreRepository.getAllFourXFourBanners();
      log(
        'Received API response for 4x4 banners. Data present: ${response.values != null}',
      );
      log('Number of 4x4 banners: ${response.values?.length ?? 0}');
      ...
      if (response.values != null && response.values!.isNotEmpty) {
        ...
        // Process only the first 4 banners (for 4x4 layout)
        final bannersToProcess = response.values!.take(4);
        ...
        for (var banner in bannersToProcess) {
          ...
          if (banner.slidePosition != null) {
            if (banner.slidePosition == 'Home Page Banner 1') {
              bannerIndex = 0;
            } else if (banner.slidePosition == 'Home Page Banner 2') {
              bannerIndex = 1;
            } else if (banner.slidePosition == 'Home Page Banner 3') {
              bannerIndex = 2;
            } else if (banner.slidePosition == 'Home Page Banner 4') {
              bannerIndex = 3;
            }
          }
      ...
      // Add default slides if needed to reach 4 slides total
      while (staticSlides.length < 4) {
      ...
      log('Error occurred, added 4 default empty slides with custom positions');
    } finally {
  ...
  Future<void> saveChanges() async {
    try {
      ...
      showSuccessToast(message: "4x4 banners saved successfully");
    } catch (e) {
  ...
  Future<void> _uploadSlideImage(String slideId, File file) async {
    try {
      ...
      final presignedRequest = BannersFourXFourImagePresignedUrlRequest(
        images: images,
      );

      final presignedResponse = await _webstoreRepository
          .bannersFourXFourImagePresignedUrl(presignedRequest);
      ...
      if (presignedResponse.images != null &&
        ...
        if (imageInfo.presignedUrl != null) {
          await _webstoreRepository.putBannerFourXFourImages(
  ...
  Future<void> _updateBanners() async {
    try {
      List<UpdateBannersFourXFourRequest> updateRequests = [];
      ...
      for (var slide in slides) {
        ...
        if (originalBannerData.containsKey(slideId)) {
          ...
          if (clearedImages.contains(slideId)) {
            // Keep images list empty for cleared images
          ...
          if (uploadedImageInfo.containsKey(slideId)) {
            final imageInfo = uploadedImageInfo[slideId]!;
            final bannerImage = UpdateBannersFourXFourRequestImage(
              s3Key: imageInfo['s3Key'],
              fileName: imageInfo['fileName'],
              fileType: imageInfo['fileType'],
            );
      ...
      if (updateRequests.isNotEmpty) {
        ...
        await _webstoreRepository.updateBannerFourXFour(updateRequests);
      } else {
