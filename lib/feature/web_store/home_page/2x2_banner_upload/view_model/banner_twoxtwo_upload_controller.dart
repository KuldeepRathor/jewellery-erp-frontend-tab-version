...
class BannerTwoXTwoUploadController extends GetxController {
  ...
  final bannersResponse = Rx<ApiResponse<GetAllBannersTwoXTwoResponse>>(
  ...
  final RxMap<String, GetAllBannersTwoXTwoValue> originalBannerData =
      <String, GetAllBannersTwoXTwoValue>{}.obs;
  ...
  Future<void> fetchBanners() async {
    try {
      ...
      final response = await _webstoreRepository.getAllTwoXTwoBanners();
      log(
        'Received API response for 2x2 banners. Data present: ${response.values != null}',
      );
      log('Number of 2x2 banners: ${response.values?.length ?? 0}');
      ...
      if (response.values != null && response.values!.isNotEmpty) {
        ...
        // Process only the first 2 banners (for 2x2 layout)
        final bannersToProcess = response.values!.take(2);
      ...
      // Add default slides if needed to reach 2 slides total
      while (staticSlides.length < 2) {
      ...
      log('Error occurred, added 2 default empty slides with custom positions');
    } finally {
  ...
  Future<void> saveChanges() async {
    try {
      ...
      showSuccessToast(message: "2x2 banners saved successfully");
    } catch (e) {
  ...
  Future<void> _uploadSlideImage(String slideId, File file) async {
    try {
      ...
      final presignedRequest = BannersTwoXTwoImagePresignedUrlRequest(
        images: images,
      );

      final presignedResponse = await _webstoreRepository
          .bannersTwoXTwoImagePresignedUrl(presignedRequest);
      ...
      if (presignedResponse.images != null &&
        ...
        if (imageInfo.presignedUrl != null) {
          await _webstoreRepository.putBannerTwoXTwoImages(
  ...
  Future<void> _updateBanners() async {
    try {
      List<UpdateBannersTwoXTwoRequest> updateRequests = [];
      ...
      for (var slide in slides) {
        ...
        if (originalBannerData.containsKey(slideId)) {
          ...
          if (uploadedImageInfo.containsKey(slideId)) {
            final imageInfo = uploadedImageInfo[slideId]!;
            final bannerImage = UpdateBannersTwoXTwoRequestImage(
              s3Key: imageInfo['s3Key'],
              fileName: imageInfo['fileName'],
              fileType: imageInfo['fileType'],
            );
      ...
      if (updateRequests.isNotEmpty) {
        ...
        await _webstoreRepository.updateBannerTwoXTwo(updateRequests);
      } else {
