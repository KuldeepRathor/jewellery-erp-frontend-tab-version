...
class NewBookingPaymentDetailsController extends GetxController {
  final JewelleryPlanRepository _repository = JewelleryPlanRepository();
  final NewBookingController newBookingController =
      Get.put<NewBookingController>(NewBookingController());
  ...
  void onInit() {
    super.onInit();
    bookingAmount =
        newBookingController.advancePayableAmountController.text.isEmpty
            ? "0"
            : newBookingController.advancePayableAmountController.text;
  ...
  Future<bool> validateAndCreateSubscription() async {
    try {
      ...
      double targetAmount = double.tryParse(bookingAmount ?? "0") ?? 0;
      log("Target Amount: $targetAmount, Total: $total");

      if (total != targetAmount) {
        showErrorToast(
          message: "Total payment amount must match advance payable amount",
      ...
      final currentRate =
          double.tryParse(
            goldRateController.getRateForPurity('22k').replaceAll(',', ''),
          ) ??
          0;
      ...
      // Create the request
      final request = NewBookingRequest(
        setupId: newBookingController.activeBookingRule.value?.id,
        phone:
            partyDetailsController.selectedParty.value?.phoneNumber ??
            partyDetailsController.selectedParty.value?.mobile,
        paymentOptions: [
          PaymentOption(
            type: controllers.first.method.text,
            options: paymentOptions,
            mode: methodToMode[controllers.first.method.text],
          ),
        ],
        weight: newBookingController.weightController.text,
        payable: int.tryParse(
          newBookingController.advancePayableAmountController.text,
        ),
        rate: currentRate.round(),
      );

      log("Request payload: ${request.toJson()}");

      final response = await _repository.newBooking(request);
      log("Booking response: $response");

      showSuccessToast(message: 'Booking created successfully');
      clearAllControllers();
      Get.back();
      Get.back();
      return true;
      ...
      showErrorToast(message: "Failed to create booking: ${e.toString()}");
      return false;
    } finally {
  ...
  void clearAllControllers() {
    ...
    newBookingController.clearControllers();
  }
}
