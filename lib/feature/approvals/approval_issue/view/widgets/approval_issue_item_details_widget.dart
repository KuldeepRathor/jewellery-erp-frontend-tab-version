...
class ApprovalIssueItemDetailsWidget extends StatefulWidget {
  const ApprovalIssueItemDetailsWidget({super.key});

  @override
  State<ApprovalIssueItemDetailsWidget> createState() =>
      _ApprovalIssueItemDetailsWidgetState();
}

class _ApprovalIssueItemDetailsWidgetState
    extends State<ApprovalIssueItemDetailsWidget> {
  final ApprovalIssueItemDetailsController controller = Get.put(
    ...
    if (event is KeyDownEvent) {
      if (event.logicalKey == LogicalKeyboardKey.enter) {
        if (HardwareKeyboard.instance.isShiftPressed) {
          ...
          // When Enter is pressed on tag field (column index 1)
          if (controller.currentColIndex.value == 1) {
            // Check if both fields are filled
            if (controller.controllers[rowIndex].item_code.text.isNotEmpty &&
                controller.controllers[rowIndex].tag_no.text.isNotEmpty) {
              // Fetch the data
  ...
  TableRow _buildTableHeaders(ApprovalIssueItemDetailsController controller) {
    List<Widget> cells = [];
  ...
  List<TableRow> _buildRows(ApprovalIssueItemDetailsController controller) {
  ...
  TableRow _buildTableRow(
    int index,
    ApprovalIssueItemDetailsController controller,
  ) {
    List<Widget> cells = [
      _buildCell(
  ...
  Widget _buildCell({
    ...
    bool editable = true,
    int rowIndex = 0,
    required ApprovalIssueItemDetailsController controller,
    FocusNode? focusNode,
  }) {
    return Container(
      child:
