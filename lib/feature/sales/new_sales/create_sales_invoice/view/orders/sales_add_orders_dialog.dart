...
class SalesAddOrdersDialog extends StatefulWidget {
  const SalesAddOrdersDialog({super.key});

  @override
  State<SalesAddOrdersDialog> createState() => _SalesAddOrdersDialogState();
}

class _SalesAddOrdersDialogState extends State<SalesAddOrdersDialog> {
  final SalesAddOrdersDialogController controller =
              ...
              child: CustomTextField(
                ...
                validator: (value) {
                  if (controller.selectedOrders.isEmpty) {
                    return 'Please add at least one order';
                  }
                  return null;
                },
              ),
                ...
                SizedBox(
                  ...
                  child: Obx(
                    () => CustomButton1(
                      ...
                      onTap: controller.fetchOrdersByOrderNumber,
                    ),
                  ),
                ),
              ],
  ...
  Widget _buildFetchedBookingDetails() {
    return Obx(
      () => Visibility(
        visible:
            controller.getOrdersByOrderNumberResponse.value.status ==
            Status.COMPLETED,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Obx(
              () => _buildBookingDetailsCard(
                controller.getOrdersByOrderNumberResponse.value,
                showAddButton: true,
              ),
            ),
            const SizedBox(height: 16),
  ...
  Widget _buildSelectedBookings() {
    return Obx(
      () => Visibility(
        visible: controller.selectedOrders.isNotEmpty,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(
            ...
            Column(
              children:
                  controller.selectedOrders.map((booking) {
                    return Padding(
  ...
  Widget _buildSelectedBookingCard(OrdersByOrderNumberResponse booking) {
    return Container(
  ...
  Widget _buildBookingDetailsRow(OrdersByOrderNumberResponse booking) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        _buildDetailColumn('Order ID', booking.orderNumber ?? ''),
        // _buildDetailColumn('Booking Rate/gms', '${booking.rateValue}'),
        // _buildDetailColumn('Weight (gms)', '${booking.quantity}'),
        _buildDetailColumn(
          'Amount',
          '${booking.paymentDetails?.firstOrNull?.finalAmount}',
        ),
        _buildDetailColumn(
          'Received Amount',
          '₹${booking.paymentDetails?.firstOrNull?.receivedAmount}',
        ),
      ],
    );
  }
  ...
  Widget _buildBookingDetailsCard(
    ApiResponse<OrdersByOrderNumberResponse> response, {
