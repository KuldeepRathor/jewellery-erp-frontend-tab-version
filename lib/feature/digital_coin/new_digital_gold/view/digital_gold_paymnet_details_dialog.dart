...
import 'package:jewellery_erp_frontend_tab_version/feature/digital_coin/new_digital_gold/view_model/digital_gold_paymnet_controller.dart';
...
class DigitalGoldPaymentDetailsTableData {
...
class DigitalGoldPaymentDetailsDialog extends StatefulWidget {
  const DigitalGoldPaymentDetailsDialog({super.key});

  @override
  State<DigitalGoldPaymentDetailsDialog> createState() =>
      _DigitalGoldPaymentDetailsDialogState();
}

class _DigitalGoldPaymentDetailsDialogState
    extends State<DigitalGoldPaymentDetailsDialog> {
  final DigitalGoldPaymentDetailsController controller = Get.put(
    DigitalGoldPaymentDetailsController(),
  );
        ...
        actions: <Type, Action<Intent>>{
          SaveStoneDetailsIntent: CallbackAction<SaveStoneDetailsIntent>(
            onInvoke: (intent) async {
              ...
              await controller.createDigitalGoldPayment();
              return;
            },
          ),
        },
  ...
  Widget _buildPaymentDetails() {
    return Column(
      ...
      children: [
        ...
        _buildPaymentRow('Weight', '${controller.weight} gm'),
        _buildPaymentRow('Rate/Gm', '₹${controller.rate}'),
        _buildPaymentRow('Sub Total', '₹${controller.subTotal}'),
        _buildPaymentRow('VA', '₹${controller.vaAmount}'),
        _buildPaymentRow('CGST', '₹${controller.cgstAmount}'),
        _buildPaymentRow('SGST', '₹${controller.sgstAmount}'),
        _buildPaymentRow('IGST', '₹${controller.igstAmount}'),
        _buildPaymentRow('Round Off', '₹${controller.roundOff}'),
        _buildPaymentRow(
          'Total Amount',
        ...
        // CustomTextField(
        //   controller: controller.installmentAmountController,
        // )
      ],
    );
  }
      ...
      child: Row(
        ...
        children: [
          Material(
            ...
            child: InkWell(
              onTap: () => controller.createDigitalGoldPayment(),
  ...
  Widget _buildDropdown({
    ...
    return Padding(
      ...
      child: Focus(
        ...
        child: Obx(() {
          final accountsStatus = controller.accountsResponse.value.status;
          final isLoading = accountsStatus == Status.LOADING;
          final hasError = accountsStatus == Status.ERROR;
          ...
          // If list is empty, show a disabled dropdown
          if (list.isEmpty) {
            return const SizedBox(
              height: 38,
