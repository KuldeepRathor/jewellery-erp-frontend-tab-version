...
class PosAccount {
  ...
  PosAccount({required this.id, required this.name, required this.type});
}

class AccountPaymentViewmodel extends GetxController {
  ...
  Future<void> fetchPaymentNumber() async {
    try {
      final recordNumber = await _purchaseRepository.coreNextSequence(
        invoiceType: "payment_number",
      );
      paymentNumber.value = recordNumber;
    } catch (e) {
      log('Error fetching payment number: $e');
  ...
  Future<void> getPartyDetailsById({
    ...
    try {
      ...
      final AccountsPaymentLineItemController controller =
      ...
      try {
        final AccountPaymentDetailsController paymentController =
            Get.find<AccountPaymentDetailsController>();
