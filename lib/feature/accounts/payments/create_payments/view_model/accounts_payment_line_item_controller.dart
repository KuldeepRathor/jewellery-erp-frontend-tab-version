...
class AccountsPaymentLineItemData {
  String? id;
  ...
  factory AccountsPaymentLineItemData.fromPartyDetail(
    ...
    return AccountsPaymentLineItemData(
      id: detail.invoiceId,
...
class AccountsPaymentLineItemController extends GetxController {
  final headers = [
    'Sn',
    'Invoice Date',
    'Invoice Amount',
  ...
  final RxList<AccountsPaymentLineItemData> items =
      <AccountsPaymentLineItemData>[
        // AccountsPaymentLineItemData(
  ...
  void updateItemsFromResponse(GetPartyDetailsByLedgerResponse response) {
    List<AccountsPaymentLineItemData> newItems = [];

    // Add purchase details
    if (response.purchaseDetails != null) {
      newItems.addAll(
        response.purchaseDetails!.map(
          (detail) => AccountsPaymentLineItemData.fromPartyDetail(detail),
        ),
      );
    }

    // Add sales return details
    if (response.salesReturnDetails != null) {
      newItems.addAll(
        response.salesReturnDetails!.map(
          (detail) => AccountsPaymentLineItemData.fromPartyDetail(detail),
        ),
      );
    }
