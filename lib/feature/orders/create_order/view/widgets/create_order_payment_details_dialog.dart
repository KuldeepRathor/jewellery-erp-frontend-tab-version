...
class CreateOrderPaymentDetailsTableData {
  ...
  CreateOrderPaymentDetailsTableData({
...
class CreateOrderPaymentDetailsDialog extends StatefulWidget {
  const CreateOrderPaymentDetailsDialog({super.key});

  @override
  State<CreateOrderPaymentDetailsDialog> createState() =>
      _CreateOrderPaymentDetailsDialogState();
}

class _CreateOrderPaymentDetailsDialogState
    extends State<CreateOrderPaymentDetailsDialog> {
  final CreateOrderPaymentDetailsController controller = Get.put(
    CreateOrderPaymentDetailsController(),
  );

  final CreateOrderItemDetailsController itemDetailsController =
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Request focus on the paid amount field
        ...
        actions: <Type, Action<Intent>>{
          ...
          SaveStoneDetailsIntent: CallbackAction<SaveStoneDetailsIntent>(
            onInvoke: (intent) async {
              ...
              await controller.validateAndSaveOrder();
              return;
            },
          ),
        },
    ...
    return Column(
      ...
      children: [
        Obx(
          () => _buildPaymentRow(
            'Approx Amount',
            '₹ ${controller.getSubTotalWithoutGST()}',
          ),
        ),
        ...
        Obx(() {
          double oldGoldTotal = controller.getOldGoldTotal();
          if (oldGoldTotal > 0) {
            return Column(
              children: [
                // _buildPaymentRow('Total Before Old Gold',
                //     '₹ ${controller.getTotalBeforeOldGold()}'),
                _buildPaymentRow(
                  'Old Gold',
        ...
        Obx(
          () => _buildPaymentRow('Total', '₹ ${controller.totalPrice.value}'),
        ),
        const MySeparator(height: 3, color: grey1),
        _buildTextfieldPaymentRow(
          label: 'Advance Amount',
          textController: controller.paidAmountController,
          focusNode: controller.paidAmountFocusNode,
          onChanged: (p0) {
      ...
      child: Row(
        ...
        children: [
          Material(
            ...
            child: InkWell(
              onTap: () => controller.validateAndSaveOrder(),
