...
class QuickGlobalOldGoldDialog extends StatefulWidget {
  const QuickGlobalOldGoldDialog({super.key});

  @override
  State<QuickGlobalOldGoldDialog> createState() =>
      _QuickGlobalOldGoldDialogState();
}

class _QuickGlobalOldGoldDialogState extends State<QuickGlobalOldGoldDialog> {
  final QuickGlobalOldGoldDialogController controller =
      Get.find<QuickGlobalOldGoldDialogController>();
  final GoldRateController goldRateController = Get.find<GoldRateController>();
    ...
    if (event is KeyDownEvent) {
      ...
      if (event.logicalKey == LogicalKeyboardKey.enter) {
        if (HardwareKeyboard.instance.isShiftPressed) {
          ...
          // ADDED: Check if we're leaving the purity field
      ...
      // ADDED: Handle escape key to reset row
      if (event.logicalKey == LogicalKeyboardKey.escape) {
        ...
        actions: <Type, Action<Intent>>{
          SaveStoneDetailsIntent: CallbackAction<SaveStoneDetailsIntent>(
            onInvoke: (intent) async {
              ...
              await controller.validateAndSubmitOldGold();
  ...
  Widget _buildCodeDropdown({
    required int index,
    required QuickGlobalOldGoldDialogController controller,
      ...
      return GenericAutocompleteDropdown<GetAllOrnamentsResponseValue>(
        ...
        onSelected: (selectedValue) {
          if (selectedValue.code != null) {
            ...
            controller.controllers[index].ornamentId = selectedValue.id!;
  ...
  //         final rate = goldRateController.getRateForPurity(selectedValue);
  ...
  Widget _buildCell({
    ...
    Function(bool)? onFocusChange, // ADDED: New parameter
  }) {
    return Container(
      child:
          editable
              ? Padding(
                ...
                child: Focus(
                  ...
                  // UPDATED: Combine focus change handling
                  onFocusChange: (hasFocus) {
                    if (hasFocus &&
                        selectAllOnTap &&
                        textController != null &&
                    ...
                    // ADDED: Call custom onFocusChange if provided
                  ...
                  child: TextFormField(
                    ...
                    onTap: () {
                      ...
                      // Select all text if this is a field that should select all on tap
                      if (selectAllOnTap &&
                          textController != null &&
                          textController.text.isNotEmpty) {
                        // Small delay to ensure the tap is processed
                        Future.delayed(const Duration(milliseconds: 50), () {
                          textController.selection = TextSelection(
                            baseOffset: 0,
                            extentOffset: textController.text.length,
