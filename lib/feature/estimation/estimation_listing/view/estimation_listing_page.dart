...
import 'package:jewellery_erp_frontend_tab_version/feature/estimation/estimation_listing/view_model/estimation_listing_controller.dart';
...
class EstimationListingPage extends StatefulWidget {
  const EstimationListingPage({super.key});

  @override
  State<EstimationListingPage> createState() => _EstimationListingPageState();
}

class _EstimationListingPageState extends State<EstimationListingPage> {
  final EstimationListingController controller = Get.put(
    EstimationListingController(),
  );
  ...
  void initState() {
    super.initState();
    // controller.getApprovalListingDetails();
    ...
    return Scaffold(
      ...
      body: PermissionGuard(
        ...
        child: ActionScopeWidget(
          onNewButtonTap: addNewEstimateTap,
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              HeaderWidget(header: 'Estimation List'),
              Expanded(
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildEstimationListTable(controller),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildActionBar(BuildContext context) {
    return Row(
      children: [
        ...
        //   buttonName: 'Correction (Ctrl + F2)',
        // ),
        const SizedBox(width: 20),
        PermissionGuard(
          actionCode: 2101,
          child: CustomButton2(
            onTap: addNewEstimateTap,
            image: 'assets/svgs/add.svg',
            buttonName: 'Add New Estimate',
          ),
        ),
      ],
    );
  ...
  void addNewEstimateTap() {
    PermissionGuardUtil.withActionPermission(2101, () {
      SidebarController sidebarController = Get.find<SidebarController>();
      sidebarController.navigateToWidget(
        newChild: const EstimationPage(initialTabIndex: 1),
      );
    });
  }
  ...
  Widget _buildEstimationListTable(EstimationListingController controller) {
    return Expanded(
      child: Container(
